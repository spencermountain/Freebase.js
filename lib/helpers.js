...
module.exports = {
  ...
  singularize: function(text) {
    if(text.match(' ')) { //multiple words
      ...
      return firsts.join(" ") + ' ' + module.exports.singularize(last);
    }
    var prepositions = {
      "about": 1,
      "above": 1,
  ...
  doit_async: function(arr, fn, options, done) {
    ...
    var all = module.exports.groups_of(function_list, async_max).map(function(f_group) {
      return function(callback) {
  ...
  groups_of: function(arr, group_length) {
    var all = []
    for(var i in arr) {
      if(i % group_length == 0) {
        all.push([arr[i]])
